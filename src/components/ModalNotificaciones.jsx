      ...
      <div className='bg-white rounded-lg shadow-2xl sm:w-full md:w-1/3 mt-4 max-h-screen overflow-y-auto mr-4 border border-gray-200 relative'>
        ...
        <div className='flex flex-col p-4 space-y-4 mb-4'>
          <div className='overflow-y-auto max-h-60'>
            {loading ? (
              <p className='text-lg text-center text-gray-700'>Cargando notificaciones...</p>
            ) : notificaciones.length === 0 ? (
              <p className='text-lg text-center text-gray-700'>No hay notificaciones.</p>
            ) : (
              notificaciones.map((notificacion) => (
                <div key={notificacion.id} className='border-b border-gray-200 py-2 flex items-center'>
                  <div className={`w-2 h-2 rounded-full mr-4 ${notificacion.nueva ? 'bg-blue-500' : 'bg-transparent'}`}></div>
                  <div className='flex-1'>
                    <p className='text-lg'>{notificacion.mensaje}</p>
                    <p className='text-base text-gray-500'>{notificacion.fecha}</p>
                  </div>
                </div>
              ))
            )}
