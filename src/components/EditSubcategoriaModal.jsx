...
const EditSubcategoriaModal = ({ isOpen, onClose, subcategoria }) => {
    ...
    const fetchsubcategoriaDetails = async (subcategoriaId) => {
        ...
        try {
            ...
            if (response.status === 200) {
                ...
                console.error('Error fetching subcategoria details:', response.data.message);
                toast.error('Error al cargar la información del subcategoria.', { position: 'top-right' });
                setLoading(false);
            }
        } catch (error) {
    ...
    const validateInput = (name, value) => {
        ...
        if (name === 'subcategoriaName') {
            ...
            if (!nameRegex.test(value) || /\d/.test(value)) {
                errorMessage = 'El subcategoriaName no puede contener caracteres especiales.';
            }
        } 
        return errorMessage;
    };
    ...
    const handleUpdate = async () => {
        ...
        setLoading(true);
        try {
            const response = await api.put(`/subcategoria/${subcategoria.id}`, {
                subcategoriaName,
                CategoriaId: CategoriaId,
                EstadoId: EstadoId,
            }, {
            ...
            if (response.status === 200) {
                ...
                console.error('Error updating subcategoria profile:', response.data.message);
                toast.error('Error al actualizar la información del subcategoria.', { position: 'top-right' });
            }
        } catch (error) {
                    ...
                    <div className='w-full md:w-3/4'>
                            <div className='font-inter ml-2'>
                                <div className='space-y-2 md:space-y-2 text-left'>
                                    <h6 className='font-bold text-center text-2xl mb-2'>Editar Subcategoria</h6>

                                    <div className='flex flex-col'>
                                        <label className='mb-1 font-bold text-sm'>Subcategoria *</label>
                                        <input
                                        ...
                                        />
                                        {formErrors.subcategoriaName && <div className='text-red-400 text-sm mt-1 px-2'>{formErrors.subcategoriaName}</div>}
                                    </div>                                 
